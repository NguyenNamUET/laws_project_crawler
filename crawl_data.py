from my_crawler.sitemap_helper import get_all_sitemaps_url, get_all_document_url, get_url_from_page__vbpl
from my_crawler.multithread_helper import multithread_crawl_with_batch, multithread_with_ouput
from my_crawler.url_helper import load_url, get_id_from_url, get_id_from_url__vbpl
from my_crawler.document_vbpl_transform import get_page_count
...
from utilities.email_me import send_email
...
def store_sitemaps_and_urls__vbpl(root_path):
    ...
    for index, (name, url) in enumerate(list(VBPL_SITEMAP.items())[45:46]):
        ...
        for url_list in doc_urls_per_page_list:
            for url in url_list:
                write_to_record(url, root_path + FILE_VERSION + "/urls/urls.lines", by_line=True, is_append=True)
...
def store_sitemaps_and_urls__thuvienphapluat(root_path):  # , sitemap_dir, url_file
    ...
    for sitemap_url in tqdm(sitemap_urls[10:11], total=len(sitemap_urls[10:11])):
        file_name = root_path + FILE_VERSION + "/sitemaps/sitemaps_part" + re.findall("(\d+)", sitemap_url)[0] + ".xml"
        ...
        for document_url in document_urls:
            write_to_record(document_url, root_path + FILE_VERSION + "/urls/urls.lines", by_line=True, is_append=True)
...
def get_uncrawled_urls_compared_to_version(version_path):
    crawled_urls_id_from_version = get_crawled_document_list(version_path)
    if crawled_urls_id_from_version is not None:
        current_urls_path = "/".join(version_path.split("/")[:-1]) + "/" + FILE_VERSION + "/urls/urls.lines"
...
def crawl(root_path, keep_record, check_duplicate,
    ...
    if mode == "manual":
        ...
        if choice_input == "1":
            ...
            if choosen_version is not None:
                ...
                url_list = load_record_to_list(root_path + FILE_VERSION + "/urls/urls.lines")
    ...
    if url_list is not None and len(url_list) > 0:
        ...
        multithread_crawl_with_batch(document_urls=url_list,
                                     current_version_path=root_path + FILE_VERSION,
                ...
                while True:
                    ...
                    if merge_input != "Y" and merge_input != "y" and merge_input != "N" and merge_input != "n":
                        ...
                        if merge_input == "Y" or merge_input == "y":
                            merge_two_version(choosen_version, root_path + FILE_VERSION)
                        break
        ...
        shutil.rmtree(root_path + FILE_VERSION)
...
if __name__ == "__main__":
    while True:  # remove loop for manual
        ...
        timer_extract_first = store_sitemaps_and_urls__thuvienphapluat(THUVIENPHAPLUAT_ROOTPATH)
        ...
        if is_sleppy_1 == 0 or is_sleppy_2 == 0:
            print("Đang ngủ...")
            countdown(172800) #sleep for 2 days
